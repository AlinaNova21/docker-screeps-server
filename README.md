...
```bash
docker run -it --rm -v $PWD:/screeps juligreen/screeps-server init
```
...
2. Run the server
```bash
docker run -d --name screeps-server -v $PWD:/screeps -p 21025:21025 juligreen/screeps-server
```
...
```docker run --rm -v $PWD:/screeps juligreen/screeps-server yarn add screepsmod-auth```
...
```docker run -d --name screeps-server-backend -v $PWD:/screeps juligreen/screeps-server@beta screeps-backend```
